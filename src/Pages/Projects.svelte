<script lang="ts">
  import { blur } from 'svelte/transition';
  export let self, projects;
</script>

<svelte:head>
  <title>Projects | {self.name}</title>
</svelte:head>
<section
  class="container section section__height"
  id="projects"
  transition:blur={{ duration: 500 }}
>
  <h2 class="section__title">Projects</h2>
  <hr />
  <p id="description">Some projects that i've been working on</p>
  {#each projects as { name, description, repo_name, demo_url, img_preview, emoji }, i}
    <div class="list">
      <h1>{i + 1}. {emoji} {name}</h1>
      <p>{description}</p>
      <p>
        <a href={demo_url} target="_blank">Demo</a> |
        <a href="https://github.com/gifaldyazkaa/{repo_name}" target="_blank"
          >View on GitHub</a
        >
        |
        <a href={img_preview} target="_blank">Preview</a>
      </p>
    </div>
  {/each}
  <p id="info">
    These projects are listed depending on GitHub Pins at <a
      href="https://github.com/gifaldyazkaa"
      target="_blank">My GitHub Profile</a
    >.
  </p>
</section>

<style>
  /* Base */
  h1,
  h2,
  p {
    cursor: default;
  }
  a {
    color: rgb(18, 18, 211);
  }
  a:hover {
    color: rgb(13, 13, 118);
  }
  a:visited {
    color: rgb(18, 18, 211);
  }
  /* Main section */
  .section__title,
  #description {
    font-family: 'Lexend', sans-serif;
  }
  #description {
    margin-top: 15px;
    margin-bottom: 15px;
  }
  .list {
    margin-bottom: 15px;
  }
  #info {
    padding-bottom: 90px;
    font-size: 10px;
    font-family: 'Lexend', sans-serif;
  }
</style>
